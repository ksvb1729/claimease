...
function QrBlock({ payloads }: { payloads: string[] }) {
  return (
...
function PageFrame({ template, fallbackTemplate, qrPayloads, children }: {
  template: string; fallbackTemplate?: string;
  qrPayloads: string[]; children?: React.ReactNode;
}) {
  const [activeTemplate, setActiveTemplate] = useState(template);
  return (
    <section className="a4-page page-break">
      <img src={activeTemplate} alt="IRDAI claim form" className="form-template"
        onError={() => { if (fallbackTemplate && activeTemplate !== fallbackTemplate) setActiveTemplate(fallbackTemplate); }} />
      <div className="page-overlay">
        <QrBlock payloads={qrPayloads} />
        {children}
      </div>
    </section>
  );
...
export default function FormRenderer({ data }: Props) {
  ...
  const qrPayloads = buildQrPayloads(data);

  return (
    <div className="form-preview-wrap">
      {/* Part A — patient fills */}
      <PageFrame template={claimFormPartA} fallbackTemplate={claimFormPartA} qrPayloads={qrPayloads}>
      ...
      {/* Part A guidance page — no overlay */}
      <PageFrame template="/src/assets/claim-form-part-a-guidance.png" fallbackTemplate={claimFormPartA} qrPayloads={qrPayloads} />

      {/* Part B — hospital fills; pre-filled from documents */}
      <PageFrame template={claimFormPartB} fallbackTemplate={claimFormPartB} qrPayloads={qrPayloads}>
        {/* Hospital details */}
        <Text x={30} y={8.5} w={55} text={data.hospitalName} />
        <Text x={14} y={12} w={70} text={data.hospitalAddress} size={5.5} />
        <Text x={30} y={14.8} w={25} text={data.hospitalPhone} />
        <Text x={14} y={17.8} w={45} text={data.hospitalEmail} size={5.5} />
        ...
        <Text x={70} y={22.8} w={18} text={data.treatingDoctorQualification} size={5.5} />
        {/* Diagnosis & procedure */}
        <Text x={25} y={29.5} w={50} text={data.diagnosisText} size={5.5} />
        <Text x={80} y={29.5} w={12} text={data.diagnosisIcdCode} boxed />
        <Text x={25} y={33.5} w={50} text={data.procedureName} size={5.5} />
        <Text x={80} y={33.5} w={12} text={data.procedureIcdCode} boxed />
        {/* Dates — from Part A data */}
        ...
        {/* Non-network bank details */}
        <Text x={14} y={67} w={50} text={data.hospitalAddress} size={5.5} />
      </PageFrame>

      {/* Part B guidance page — no overlay */}
      <PageFrame template="/src/assets/claim-form-part-b-guidance.png" fallbackTemplate={claimFormPartB} qrPayloads={qrPayloads} />
    </div>
  );
}
